...
long getCpuTime(int dims, int popSize, int interation) {
  std::cout << "Please wait --> running DE on CPU" << std::endl;
  using namespace std::chrono;
  auto cpu_tick = high_resolution_clock::now();
  cpu::runRastiginOnCPU(dims, popSize, interation);
...
void printHeader(int populationSize, int maxIterations, int cpuTime) {
  ...
  std::cout << "configuration: "
            ...
            << std::setw(20) << "cpu_time: " << cpuTime << "ms" << std::right
            << "\n";
  std::cout << std::string(repeatUnderScore, '-') << std::endl;
